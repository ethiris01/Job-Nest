      ...
      isEdit ? <>
       <label htmlFor="resumeUpload" className="flex items-center">
        ...
        onChange={ e => setResume(e.target.files[0])}
         type="file" accept="application/pdf"  id="resumeUpload" hidden />
         <img src={assets.profile_upload_icon} alt="" />
       </label>
       <button
       onClick={e => setIsEdit(false)}
        className="bg-green-100 border-green-400 rounded-lg px-4 py-2">Save</button>
    ...
    <table className="min-w-full bg-white border rounded-lg">
      ...
      <tbody>
        {jobsApplied.map((job,index)=> true ? (
          ...
          <td className="py-2 px-4 border-b text-center"><span className={`${job.status === "Accepted" ? "bg-green-100" : job.status === "Rejected" ? "bg-red-100" : "bg-blue-100" } px-5 py-2 border-lg inline-block `}>{job.status}</span></td>
          </tr>
        ) : (null) )}
      </tbody>
    </table>
